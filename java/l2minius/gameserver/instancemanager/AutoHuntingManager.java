package l2minius.gameserver.instancemanager;
...
import l2minius.gameserver.randoms.CaptchaImage;
...
public class AutoHuntingManager
{
	...
	public synchronized void reportBot(Player reported, Player reporter, String typeOfReport, String moreInfo)
	{
		...
		{
			reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message1", reporter));
			return;
		}
		...
		/*
		 * if (!reported.isInCombat() && !reported.isInPeaceZone()) { reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message7", reporter));
		 * return; }
		 */
		...
		{
			reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message8", reporter));
			return;
		}
		...
		try
		{
			...
			else
			{
				...
				{
					reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message2", reporter));
					return;
				}
	...
	public boolean validateBot(Player reported, Player reporter)
	{
		...
		{
			reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message3", reporter));
			return false;
		}
		/*
		...
		{
			reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message4", reporter));
			return false;
		}
		if (reporter.getLevel() < Config.CAPTCHA_MIN_LEVEL)
		{
			reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message6", reporter));
			return false;
		}
		// Same HWID, ip
		if (reporter.isDualbox(reported))
		{
			reporter.sendMessage(new CustomMessage("l2minius.gameserver.instancemanager.autohuntingmanager.message5", reporter));
			return false;
		}
