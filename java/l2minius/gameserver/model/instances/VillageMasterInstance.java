...
import l2minius.gameserver.model.SubClass;
import l2minius.gameserver.model.base.ClassId;
import l2minius.gameserver.model.base.ClassType;
import l2minius.gameserver.model.base.PlayerClass;
...
import l2minius.gameserver.model.entity.residence.Dominion;
...
import l2minius.gameserver.network.serverpackets.PledgeReceiveSubPledgeCreated;
import l2minius.gameserver.network.serverpackets.PledgeShowInfoUpdate;
import l2minius.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import l2minius.gameserver.network.serverpackets.PledgeStatusChanged;
...
import l2minius.gameserver.utils.CertificationFunctions;
...
public final class VillageMasterInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("Subclass"))
		{
			...
			case 1: // Returns a list of subs that can be taken (see case 4)
				...
				{
					player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.NoSubAtThisTime", player));
					return;
				}
				break;
			...
			case 7: // Отмена сабкласса - обработка выбора из case 6
				...
				{
					player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.SubclassCouldNotBeAdded", player));
					return;
				}
				break;
			}
	...
	public void setLeader(Player leader, String newLeader)
	{
		...
		if (member == null)
		{
			...
			// leader.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.S1IsNotMemberOfTheClan", leader).addString(newLeader));
		...
		{
			leader.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.CannotAssignUnitLeader", leader));
			return;
		}
	...
	{
		player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.ClanLeaderWillBeChangedFromS1ToS2", player).addString(clan.getLeaderName()).addString(newLeader.getName()));
	...
	public void createSubPledge(Player player, String clanName, int pledgeType, int minClanLvl, String leaderName)
	{
		...
		if (pledgeType != Clan.SUBUNIT_ACADEMY)
		{
			...
			if (subLeader == null)
			{
				player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.PlayerCantBeAssignedAsSubUnitLeader", player));
				return;
			}
			else if (subLeader.getLeaderOf() != Clan.SUBUNIT_NONE)
			{
				player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.ItCantBeSubUnitLeader", player));
				return;
			}
		}
	...
	public void assignSubPledgeLeader(Player player, String clanName, String leaderName)
	{
		...
		if (clan == null)
		{
			player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.ClanDoesntExist", player));
			return;
		}
		...
		if (targetUnit == null)
		{
			player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.SubUnitNotFound", player));
			return;
		}
		SubUnit mainUnit = clan.getSubUnit(Clan.SUBUNIT_MAIN_CLAN);
		...
		player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.NewSubUnitLeaderHasBeenAssigned", player));
	}
	...
	public static boolean addNewSubclass(Player player, int classId)
	{
		...
		{
			player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.NoSubBeforeLevel", player).addNumber(Config.ALT_GAME_LEVEL_TO_GET_SUBCLASS));
			return false;
		}
		...
		if (!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS && !playerClassList.isEmpty() && playerClassList.size() < 2 + Config.ALT_GAME_SUB_ADD)
		{
			if (player.isQuestCompleted("_234_FatesWhisper"))
			{
				if (player.getRace() == Race.kamael)
				{
					...
					{
						player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.QuestSeedsOfChaos", player));
						return false;
					}
				}
				else if (!player.isQuestCompleted("_235_MimirsElixir"))
				{
					player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.QuestMimirsElixir", player));
					return false;
				}
			}
			else
			{
				player.sendMessage(new CustomMessage("l2minius.gameserver.model.instances.L2VillageMasterInstance.QuestFatesWhisper", player));
				return false;
			}
		}
