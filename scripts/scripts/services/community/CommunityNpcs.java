...
import l2minius.gameserver.model.instances.VillageMasterInstance;
import l2minius.gameserver.model.pledge.Clan;
import l2minius.gameserver.model.pledge.SubUnit;
import l2minius.gameserver.network.clientpackets.CharacterCreate;
import l2minius.gameserver.network.serverpackets.ConfirmDlg;
import l2minius.gameserver.network.serverpackets.HideBoard;
	...
	private static void addNewSubPage(Player player, String raceName)
	{
		...
		for (int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Add " + playerClass.name() + "\" action=\"bypass _bbsAddNewSub_" + playerClass.ordinal() + "\" width=120 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\">";
			}
		}
	...
	private static void changeSubPage(Player player)
	{
		...
		for (int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Change To " + Util.getFullClassName(ClassId.values()[playerClass.getClassId()]) + "\" action=\"bypass _bbsChangeSubTo_" + playerClass.getClassId() + "\" width=120 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\">";
			}
		}
		sendFileToPlayer(player, "smallNpcs/subclassChanger_select.htm", true, replacements);
	}
	...
	private static void cancelSubPage(Player player)
	{
		...
		for (int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Remove " + Util.getFullClassName(ClassId.values()[playerClass.getClassId()]) + "\" action=\"bypass _bbsSelectCancelSub_" + playerClass.getClassId() + "\" width=120 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\">";
			}
		}
		sendFileToPlayer(player, "smallNpcs/subclassChanger_select.htm", true, replacements);
	}
	...
	private static void chooseCertificatePage(Player player)
	{
		...
		for (int i = 0; i < FIELDS_IN_SUB_SELECT_PAGE; i++)
		{
			...
			else
			{
				...
				replacements[i * 2 + 1] = "<button value=\"Add " + button[0] + "\" action=\"bypass _bbsscripts:Util:" + button[1] + "\" width=120 height=25 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\">";
			}
		}
		sendFileToPlayer(player, "smallNpcs/subclassChanger_select.htm", true, replacements);
	}
