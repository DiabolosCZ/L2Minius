...
public class CommunityRanking implements ScriptFile, ICommunityBoardHandler
{
	...
	private void showRanking(Player player, RankingEnum type, int page)
	{
		...
		html = html.replace("%adenaBypass%", (player.getAccessLevel() > 0 ? "<td align=center><button action=\"bypass _bbsranking:adena\" value=\"Adena\" width=180 height=32 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td>" : ""));
	...
	private void showTalesRanking(Player player, RankingEnum type, int page)
	{
		...
		html = html.replace("<?update?>", String.valueOf(TIME_UPDATE / (60 * 1000)));
		html = html.replace("<?last_update?>", TIME_FORMAT.format(_lastUpdate));
		html = html.replace("%adenaBypass%", (player.getAccessLevel() > 0 ? "<tr><td height=40 align=center><button action=\"bypass _bbsranking:adena\" value=\"   Adena\" width=180 height=32 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_ct1.button_df\"></td></tr>" : ""));
		ShowBoard.separateAndSend(html, player);
	}
